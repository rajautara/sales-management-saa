            ...
            @forelse ($rebates as $rebate)
                <div class="p-4 space-y-2">
                    <div class="flex items-start justify-between gap-3">
                        <a href="{{ route('invoices.show', $rebate->invoice) }}" wire:navigate class="text-sm font-semibold text-indigo-600 hover:text-indigo-900">{{ $rebate->invoice->number }}</a>
                        <span class="text-sm font-medium text-gray-900 shrink-0">{{ number_format($rebate->amount, 2) }}</span>
                    </div>
                    <dl class="grid grid-cols-2 gap-x-3 gap-y-1 text-sm">
                        <div class="col-span-2">
                            <dt class="text-gray-500">Customer</dt>
                            <dd class="text-gray-900">{{ $rebate->invoice->customer->name }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">Date</dt>
                            <dd class="text-gray-900">{{ $rebate->date->format('Y-m-d') }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">Method</dt>
                            <dd class="text-gray-900">{{ $rebate->method->label() }}</dd>
                        </div>
                        <div class="col-span-2">
                            <dt class="text-gray-500">Reference</dt>
                            <dd class="text-gray-900">{{ $rebate->reference_no ?: '-' }}</dd>
                        </div>
                    </dl>
                    <div class="flex flex-wrap gap-x-4 gap-y-1 pt-1">
                        <button type="button" wire:click="delete({{ $rebate->id }})" wire:confirm="Delete this rebate?" class="text-sm font-medium text-red-600 hover:text-red-900 py-1">Delete</button>
                    </div>
                </div>
            @empty
                <div class="px-4 py-8 text-sm text-gray-500 text-center">No rebates found.</div>
            @endforelse
        </x-slot:cards>
