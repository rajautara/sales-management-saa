<x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ $invoiceId ? 'Edit Invoice' : 'Create Invoice' }}</h2>
</x-slot>
...
<div class="max-w-7xl mx-auto bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
    <form wire:submit="save" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
                <label class="block font-medium text-sm text-gray-700">Customer <span class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block font-medium text-sm text-gray-700">Due Date <span class="text-red-500">*</span></label>
                <input type="date" wire:model="dueDate" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1">
            ...
            <div>
                ...
                <select wire:model="status" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($items as $index => $item)
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    <input type="number" step="0.01" wire:model.live="items.{{ $index }}.discount" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-24 text-right">
                                    @error("items.{$index}.discount") <p class="text-xs text-red-600">{{ $message }}</p> @enderror
                                </td>
                                <td class="px-4 py-2">
                    ...
                    <tfoot class="bg-gray-50">
                        <tr>
                            <td colspan="6" class="px-4 py-2 text-right font-medium text-gray-700">Subtotal</td>
                            <td class="px-4 py-2 text-right font-medium text-gray-900">{{ number_format($this->totals['subtotal'], 2) }}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td colspan="6" class="px-4 py-2 text-right font-medium text-gray-700">Tax</td>
                            <td class="px-4 py-2 text-right font-medium text-gray-900">{{ number_format($this->totals['tax'], 2) }}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td colspan="6" class="px-4 py-2 text-right font-medium text-gray-700">Discount</td>
                            <td class="px-4 py-2 text-right font-medium text-gray-900">{{ number_format($this->totals['discount'], 2) }}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td colspan="6" class="px-4 py-2 text-right font-bold text-gray-900">Total</td>
                            <td class="px-4 py-2 text-right font-bold text-gray-900">{{ number_format($this->totals['total'], 2) }}</td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
        ...
        <div class="flex flex-wrap justify-end gap-3">
            <a href="{{ route('invoices.index') }}" wire:navigate class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                Cancel
            </a>
