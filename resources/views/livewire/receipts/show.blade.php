...
<div class="max-w-3xl mx-auto bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 space-y-6">
    ...
    <div class="flex justify-end gap-2">
        <a href="{{ $this->getWhatsappUrl('Receipt', $receipt->number, $receipt->payment->invoice->customer, (float) $receipt->payment->amount, URL::temporarySignedRoute('public.receipts.show', now()->addDays(7), $receipt)) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-emerald-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition ease-in-out duration-150">
