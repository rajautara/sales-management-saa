<x-public-layout>
    <x-slot name="title">Sales Order {{ $salesOrder->number }}</x-slot>

    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-8 border border-slate-100">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row justify-between items-start gap-4 pb-6 border-b border-slate-100">
            <div>
                <h1 class="text-3xl font-extrabold text-indigo-600 tracking-tight">SALES ORDER</h1>
                <p class="text-sm font-medium text-slate-500 mt-1">No: <span class="text-slate-800 font-semibold">{{ $salesOrder->number }}</span></p>
                <p class="text-sm text-slate-500">Date: <span class="text-slate-800 font-medium">{{ $salesOrder->date->format('Y-m-d') }}</span></p>
                <p class="text-sm text-slate-500">Status: <span class="text-indigo-600 font-semibold uppercase tracking-wider text-xs px-2 py-0.5 bg-indigo-50 rounded-full">{{ $salesOrder->status->label() }}</span></p>
            </div>
            <div class="text-left sm:text-right">
                <h2 class="text-lg font-bold text-slate-900">{{ $salesOrder->company->name }}</h2>
                @if ($salesOrder->company->registration_no)
                    <p class="text-xs text-slate-500">Reg No: {{ $salesOrder->company->registration_no }}</p>
                @endif
                <p class="text-sm text-slate-600 mt-1 whitespace-pre-line">{{ $salesOrder->company->address }}</p>
                @if ($salesOrder->company->phone)
                    <p class="text-sm text-slate-600">Tel: {{ $salesOrder->company->phone }}</p>
                @endif
                @if ($salesOrder->company->email)
                    <p class="text-sm text-slate-600">Email: {{ $salesOrder->company->email }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 py-6 text-sm">
            <div>
                <h3 class="font-bold text-slate-900 uppercase tracking-wider text-xs mb-2">ORDER FOR:</h3>
                <p class="font-semibold text-slate-800">{{ $salesOrder->customer->name }}</p>
                @if ($salesOrder->customer->phone)
                    <p class="text-slate-600">Tel: {{ $salesOrder->customer->phone }}</p>
                @endif
                @if ($salesOrder->customer->email)
                    <p class="text-slate-600">Email: {{ $salesOrder->customer->email }}</p>
                @endif
            </div>
            <div>
                <h3 class="font-bold text-slate-900 uppercase tracking-wider text-xs mb-2">SHIPPING ADDRESS:</h3>
                <p class="text-slate-600 whitespace-pre-line">{{ $salesOrder->customer->shipping_address ?? 'N/A' }}</p>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200 border border-slate-100 rounded-lg">
                ...
                <tfoot class="bg-slate-50">
                    <tr>
                        ...
                        <td class="px-4 py-2 text-right font-semibold text-slate-900">{{ number_format($salesOrder->subtotal, 2) }}</td>
                    </tr>
                    <tr>
                        <td colspan="5" class="px-4 py-2 text-right font-medium text-slate-600">Tax</td>
                        <td class="px-4 py-2 text-right font-semibold text-slate-900">{{ number_format($salesOrder->tax, 2) }}</td>
                    </tr>
                    <tr>
                        <td colspan="5" class="px-4 py-2 text-right font-medium text-slate-600">Discount</td>
                        <td class="px-4 py-2 text-right font-semibold text-slate-900">{{ number_format($salesOrder->discount, 2) }}</td>
                    </tr>
                    <tr class="border-t-2 border-slate-200">
                        <td colspan="5" class="px-4 py-3 text-right font-bold text-slate-900 text-lg">Total</td>
                        <td class="px-4 py-3 text-right font-extrabold text-slate-900 text-lg">{{ $salesOrder->company->currency ?? 'RM' }} {{ number_format($salesOrder->total, 2) }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
